...
| **[implementation-acl.md](implementation-acl.md)** | **Implementation plan: scoped access** — phases A0–A6, enforcement-first (the scoped repo + lint + negative tests land before any grant UI); schema/backfill, the gateway, ingest anchor, admin grant UI, onboarding + open registration (claim-personal), scoped API+UI, optional team-admin tier. |
