...
export function AssistantPanel({
  ...
  return (
    <div className="flex flex-col border-l border-slate-200 bg-gradient-to-b from-indigo-50/60 to-white">
      <div className="flex items-center gap-2 border-b border-slate-200 px-4 py-3">
        ...
        <span className="ml-auto rounded-md bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-500 ring-1 ring-slate-200">on-device</span>
      </div>

      {!message ? (
        <div className="grid flex-1 place-items-center p-6 text-center text-[12.5px] text-slate-400">
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-4">
          <Card label="Preview" labelTone="indigo">
            <p className="mt-1 text-[12.5px] leading-relaxed text-slate-600">
              <b className="text-slate-900">{senderName(message.from_addr)}</b> — {snippet(message, 220) || '(no preview)'}
            </p>
          </Card>
            ...
            <div className="mt-2 space-y-1.5">
              {['👍 Sounds good — thanks!', 'Got it, I’ll take a look.', 'Can we find time to discuss?'].map((r) => (
                <button
                  ...
                  className="block w-full truncate rounded-lg bg-slate-50 px-2.5 py-1.5 text-left text-[12px] text-slate-700 ring-1 ring-slate-200 transition hover:bg-slate-100 disabled:opacity-50"
                >
                  {r}
                </button>
              ))}
      ...
      <div className="border-t border-slate-200 p-3">
        <div className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-[12.5px] text-slate-400 ring-1 ring-slate-200">
          Ask or instruct… <span className="ml-auto text-slate-300">↵</span>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="rounded-xl bg-white p-3 shadow-sm ring-1 ring-slate-200">
      <div className="flex items-center justify-between">
        <span className={'text-[11px] font-semibold uppercase tracking-wide ' + (labelTone === 'indigo' ? 'text-indigo-600' : 'text-slate-500')}>
          {label}
        </span>
        {soon && (
...
function Todos() {
  ...
  return (
    <div className="mt-2 space-y-1.5">
      {items.map((t, i) => (
        <label key={i} className="flex items-start gap-2 text-[12.5px] text-slate-600">
